...
public class WorldModule {
	...
	public static void preInit(FMLPreInitializationEvent event) {
		...
		config.load();
		//TODO decide on ore amount
		dirtGen = config.get("oregen", "dirt", new int[] { 32, 40, 0, 256 }, "Config for underground dirt. Params: blocks per vein, chance, minY, maxY. Note that this is multiplied for each planet.").getIntList();
		gravelGen = config.get("oregen", "gravel", new int[] { 32, 20, 0, 256 }, "Config for underground gravel. Params: blocks per vein, chance, minY, maxY. Note that this is multiplied for each planet.").getIntList();
		coalGen = config.get("oregen", "coal", new int[] { 16, 30, 0, 256 }, "Config for coal ore. Params: blocks per vein, chance, minY, maxY. Note that this is multiplied for each planet.").getIntList();
		...
		lapisGen = config.get("oregen", "lapis", new int[] { 0, 0, 0, 0 }, "Config for lapis ore. Params: blocks per vein, chance, minY, maxY. Note that this is multiplied for each planet.").getIntList();
		diamondGen = config.get("oregen", "diamond", new int[] { 0, 0, 0, 0 }, "Config for diamond ore. Params: blocks per vein, chance, minY, maxY. Note that this is multiplied for each planet.").getIntList();
		emeraldGen = config.get("oregen", "emerald", new int[] { 0, 0, 0, 0 }, "Config for emerald ore. Params: blocks per vein, chance, minY, maxY. Note that this is multiplied for each planet.").getIntList();
		...
		quartzGen = config.get("oregen", "quartz", new int[] { 0, 0, 0, 0 }, "Config for quartz ore. Params: blocks per vein, chance, minY, maxY. Note that this is multiplied for each planet.").getIntList();
