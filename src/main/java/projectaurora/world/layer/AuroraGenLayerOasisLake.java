...
public class AuroraGenLayerOasisLake extends BaseGenLayer {

	public AuroraGenLayerOasisLake(long seed, BaseGenLayer layer) {
		super(seed);
	...
	public int[] getInts(World world, int x, int z, int xSize, int zSize) {
	    ...
	    for (int k1 = 0; k1 < zSize; k1++) {
	    	for (int i1 = 0; i1 < xSize; i1++) {
	    		...
	    		/*if ((biome instanceof BiomeGenDesertOasis)) {
	    			...
	    			for (int i2 = -1; i2 <= 1; i2++) {
	    				for (int k2 = -1; k2 <= 1; k2++) {
	    					...
	    					if (!(adjBiome instanceof BiomeGenDesertOasis)) {
	    						surrounded = false;
	    					}
	    				}
	    			}
	    			...
	    			if (surrounded) {
	    				newBiomeID = AuroraBiome.lake.biomeID;
	    			}
	    		}*///TODO desert oasis lake gen
